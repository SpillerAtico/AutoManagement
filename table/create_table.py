...
def get_table_3(option: int):
    ...
    for i in range(3):
        count_ship_keys = list(calculations.share_capital(balance[i], ship_count[i], option)[1].keys())
        calculate = calculations.share_capital(balance[i], ship_count[i], option)[0]
...
def get_diagrams(option: int, operand):
    if operand == 'да':
        ...
        print("Диаграммы не будут выведены")
    else:
        print('Только да или нет')
